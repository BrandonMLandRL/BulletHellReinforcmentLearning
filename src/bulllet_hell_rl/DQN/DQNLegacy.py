...
class DeepQLearning:
    ...
    # INPUTS: 
    #
    # y_true - matrix of dimension (self.batchReplayBufferSize,self.actionDimension) - this is the target 
    # y_pred - matrix of dimension (self.batchReplayBufferSize,self.actionDimension) - this is predicted by the network
    # 
    ...
    def trainingEpisodes(self):
        ...
        for indexEpisode in range(self.numberEpisodes):
            ...
            self.episodeIndex = indexEpisode

            print(f"Simulating episode {indexEpisode} out of {self.numberEpisodes}")
            
            # reset the environment at the beginning of every episode
            (currentState,_)=self.env.reset()
            currentState=self._flatten_obs(currentState)
            ...
            while not terminalState:
                ...
                env_action = self._flat_action_to_env(action)
                (nextState, reward, terminalState,_,_) = self.env.step(env_action)
                nextState=self._flatten_obs(nextState)
            ...
            # save the current state of the model in case we want to pause training
            self.mainNetwork.save(self.modelFileName)
            print(f"Updated the save file of the model: {self.modelFileName}")
